...
total_df = pd.read_csv('./한영번역/trn.csv')

total_df.rename(columns={"번역문": "SRC"}, errors="raise", inplace=True)
total_df.rename(columns={"원문": "TRG"}, errors="raise", inplace=True)
...
# 1 샘플 8196개
train_data = total_df.sample(n=1024*128, # number of items from axis to return.
...
tokenizer_kr = LTokenizer()

tokenizer_kr("내일 역 앞의 식당에서 밥 먹으러 나갈래 ?")
...
TRG_tokenizer = data.Field(sequential=True, use_vocab=True, lower=True, tokenize=tokenizer_kr,
    batch_first=True, init_token="<SOS>", eos_token="<EOS>", fix_length=DECODER_LEN)

trainset = data.TabularDataset(
        path=path+'/Translation_dataset.csv', format='csv', skip_header=False,
...
for epoch in range(N_EPOCHS):
    ...
    if train_loss < best_epoch_loss:
        ...
        torch.save(model.state_dict(), './checkpoints/aihub_1024_128_2_transformermodel.pt')
...
trained_model.load_state_dict(torch.load('checkpoints/aihub_1024_128_2_transformermodel.pt'))
...
for idx in (11, 21, 31, 41, 51):
    ...
    print("Prediction   :", predict(proprocessing(raw_src[idx])))
